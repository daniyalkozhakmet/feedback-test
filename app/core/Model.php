...
trait Model
{
	...
	public function first_by_multiple($data, $data_not = [])
	{
		$keys = array_keys($data);
		$keys_not = array_keys($data_not);
		$query = "select * from $this->table where ";
	...
	public function search($data, $data_not = [])
	{
		$keys = array_keys($data);
		$query = "select * from $this->table where ";
		$query .= ' is_accepted = 1 AND (';
		foreach ($keys as $key) {
	...
	public function search_date($data, $data_not = [])
	{
		$keys = array_keys($data);
		$query = "select * from $this->table where is_accepted = 1 AND ";

		foreach ($keys as $key) {
			$query .= $key . " >= '" . $data[$key] . " 00:00:00' AND " . $key . " <' " . $data[$key] . " 23:59:59'";
		}
			...
			foreach ($this->validationRules as $column => $rules) {
				...
				foreach ($rules as $rule) {
					...
					switch ($rule) {
						...
						case 'not_less_than_3_chars':

							if (strlen(trim($data[$column])) < 3)
								$this->errors[$column] = ucfirst($column) . " should not be less than 3 characters";
							break;
